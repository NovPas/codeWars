...
def cakes(recipe, available):
    # return int(min(available[key]/recipe[key] if key in available else 0 for key in recipe.keys()))
    return int(min(available.get(key,0)/recipe[key] for key in recipe.keys()))
